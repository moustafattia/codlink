...
private const val THEME_LOG_TAG = "CodlinkThemeManager"
private const val UI_PREFERENCES_NAME = "codlink_ui_prefs"
...
enum class CodlinkColorThemeType {
    LIGHT,
    DARK,
}

data class CodlinkThemeIndexEntry(
    val slug: String,
    val name: String,
    val type: CodlinkColorThemeType,
    val accentHex: String,
    val backgroundHex: String,
    val foregroundHex: String,
)

data class CodlinkThemeDefinition(
    val name: String,
    val type: CodlinkColorThemeType,
    val colors: Map<String, String>,
)

data class CodlinkResolvedTheme(
    val slug: String,
    val name: String,
    val type: CodlinkColorThemeType,
    ...
    companion object {
        val defaultLight =
            resolve(
                slug = "codex-light",
                definition =
                    CodlinkThemeDefinition(
                        name = "Codex Light",
                        type = CodlinkColorThemeType.LIGHT,
                        colors =
                            mapOf(
        ...
        val defaultDark =
            resolve(
                ...
                definition =
                    CodlinkThemeDefinition(
                        name = "Codex Dark",
                        type = CodlinkColorThemeType.DARK,
                        colors =
                            mapOf(
        ...
        fun resolve(
            slug: String,
            definition: CodlinkThemeDefinition,
        ): CodlinkResolvedTheme {
            val colors = definition.colors
            val background =
                colorFromHex(
                    colors["editor.background"],
                    fallback = if (definition.type == CodlinkColorThemeType.DARK) Color(0xFF111111) else Color.White,
                )
            val foreground =
                colorFromHex(
                    colors["editor.foreground"],
                    fallback = if (definition.type == CodlinkColorThemeType.DARK) Color(0xFFFCFCFC) else Color(0xFF0D0D0D),
                )
            val surface =
                colors["sideBar.background"]?.let(::colorFromHex)
                    ?: adjustBrightness(background, if (definition.type == CodlinkColorThemeType.DARK) 0.03f else -0.02f)
            val surfaceLight =
                colors["activityBar.background"]?.let(::colorFromHex)
                    ?: adjustBrightness(surface, if (definition.type == CodlinkColorThemeType.DARK) 0.04f else -0.03f)
            val accent =
                colors["textLink.foreground"]?.let(::colorFromHex)
                    ?: colors["button.background"]?.let(::colorFromHex)
                    ?: if (definition.type == CodlinkColorThemeType.DARK) Color(0xFFB0B0B0) else Color(0xFF4A4A4A)
            val accentStrong =
                colors["button.background"]?.let(::colorFromHex)
                    ?: colors["textLink.foreground"]?.let(::colorFromHex)
                    ?: accent
            val border =
                colors["editorGroup.border"]?.let(::colorFromHex)
                    ...
                    ?: adjustBrightness(surface, if (definition.type == CodlinkColorThemeType.DARK) 0.05f else -0.05f)
            val separator =
                colors["panel.border"]?.let(::colorFromHex)
                    ?: adjustBrightness(background, if (definition.type == CodlinkColorThemeType.DARK) 0.04f else -0.04f)

            return CodlinkResolvedTheme(
                slug = slug,
                ...
                border = border,
                separator = separator,
                danger = if (definition.type == CodlinkColorThemeType.DARK) Color(0xFFFF5555) else Color(0xFFD32F2F),
                success = if (definition.type == CodlinkColorThemeType.DARK) Color(0xFF6EA676) else Color(0xFF2E7D32),
                warning = if (definition.type == CodlinkColorThemeType.DARK) Color(0xFFE2A644) else Color(0xFFE65100),
...
object CodlinkThemeManager {
    private val lock = Any()
    private var appContext: Context? = null
    private var initialized = false
    private var definitionCache = LinkedHashMap<String, CodlinkThemeDefinition>()
    ...
    var lightTheme by mutableStateOf(CodlinkResolvedTheme.defaultLight)
        private set

    var darkTheme by mutableStateOf(CodlinkResolvedTheme.defaultDark)
        private set

    var activeTheme by mutableStateOf(CodlinkResolvedTheme.defaultDark)
        private set

    var themeVersion by mutableIntStateOf(0)
        private set

    var themeIndex by mutableStateOf<List<CodlinkThemeIndexEntry>>(emptyList())
        private set

    val lightThemes: List<CodlinkThemeIndexEntry>
        get() = themeIndex.filter { it.type == CodlinkColorThemeType.LIGHT }

    val darkThemes: List<CodlinkThemeIndexEntry>
    ...
    fun initialize(context: Context) {
        synchronized(lock) {
            ...
            lightTheme = loadAndResolve(selectedLightSlug) ?: CodlinkResolvedTheme.defaultLight
            darkTheme = loadAndResolve(selectedDarkSlug) ?: CodlinkResolvedTheme.defaultDark
    ...
    fun selectLightTheme(slug: String) {
        ...
        lightTheme = loadAndResolve(slug) ?: CodlinkResolvedTheme.defaultLight
        if (!systemIsDark) {
    ...
    fun selectDarkTheme(slug: String) {
        ...
        darkTheme = loadAndResolve(slug) ?: CodlinkResolvedTheme.defaultDark
        if (systemIsDark) {
    ...
    private fun loadThemeIndex(): List<CodlinkThemeIndexEntry> {
    ...
    private fun loadAndResolve(slug: String): CodlinkResolvedTheme? {
        val definition = loadDefinition(slug) ?: return null
        return CodlinkResolvedTheme.resolve(slug = slug, definition = definition)
    }

    private fun loadDefinition(slug: String): CodlinkThemeDefinition? {
    ...
    private fun parseThemeDefinition(json: JSONObject): CodlinkThemeDefinition {
...
private fun String.toThemeType(): CodlinkColorThemeType =
    if (equals("light", ignoreCase = true)) {
        CodlinkColorThemeType.LIGHT
    } else {
        CodlinkColorThemeType.DARK
    }
