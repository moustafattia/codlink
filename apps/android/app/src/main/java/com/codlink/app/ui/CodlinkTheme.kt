...
object CodlinkTheme {
    private val activeTheme: CodlinkResolvedTheme
        get() = CodlinkThemeManager.activeTheme

    val themeKey: String
        get() = activeTheme.slug

    val isDark: Boolean
        get() = activeTheme.type == CodlinkColorThemeType.DARK
    ...
    val monoFont: FontFamily
        get() = if (CodlinkThemeManager.monoFontEnabled) BerkeleyMono else FontFamily.Monospace
    ...
    val backgroundBrush: Brush
        get() =
            ...
            colors =
                listOf(
                    background,
                    CodlinkResolvedTheme.adjustBrightness(
                        background,
...
@Composable
fun CodlinkAppTheme(content: @Composable () -> Unit) {
    val appContext = LocalContext.current.applicationContext
    DisposableEffect(appContext) {
        CodlinkThemeManager.initialize(appContext)
        onDispose {}
    }

    val darkModeEnabled = CodlinkThemeManager.darkModeEnabled
    val lightThemeSlug = CodlinkThemeManager.lightTheme.slug
    val darkThemeSlug = CodlinkThemeManager.darkTheme.slug
    LaunchedEffect(darkModeEnabled, lightThemeSlug, darkThemeSlug) {
        CodlinkThemeManager.applySystemTheme(darkModeEnabled)
    }

    val activeTheme = CodlinkThemeManager.activeTheme
    val colorScheme =
        remember(activeTheme.slug, activeTheme.type) {
            if (activeTheme.type == CodlinkColorThemeType.DARK) {
                darkColorScheme(
    ...
    val monoFontEnabled = CodlinkThemeManager.monoFontEnabled
...
@Composable
private fun CodlinkThemePreview() {
    CodlinkAppTheme {
        Surface(color = CodlinkTheme.background) {
            Text(
                text = "Codlink Theme",
